...
public class FoodDAO extends DBContext {
    ...
    public int getNumFoodAC(int cinID) {
        int a = 0, b = 0;
        try {
            String sql = "SELECT SUM(Quantity) AS T FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND TransactionCode.cinID = ?";
            String sql1 = "SELECT SUM(Quantity) AS T FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID WHERE cinID = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, cinID);
            ResultSet rs = st.executeQuery();
            if (rs.next()) {
    ...
    public int getNumFoodByDateAC(Date dS, Date eS, int cinID) {
        int a = 0, b = 0;
        try {
            String sql = "SELECT SUM(Quantity) AS T FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID  JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND (PaymentDate BETWEEN ? AND ?) AND TransactionCode.cinID = ?";
            String sql1 = "SELECT SUM(Quantity) AS T FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID WHERE (Date BETWEEN ? AND ?) AND cinID = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setDate(1, dS);
            st.setDate(2, eS);
            st.setInt(3, cinID);
    ...
    public List<Food> getAllFoodTypeByDateAC(Date dS, Date eS, int cinID) {
        List<Food> listF = new ArrayList<>();
        try {
            String sql = "SELECT DISTINCT ftID, ftName FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID JOIN Food ON OrderOnlineDetail.ProductCode = Food.ProductCode JOIN FoodType ON Food.FoodType = FoodType.ftID  JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND (PaymentDate BETWEEN ? AND ?) AND TransactionCode.cinID = ?\n"
                    + "UNION\n"
                    + "SELECT DISTINCT ftID, ftName FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID JOIN Food ON OrderOfflineDetail.ProductCode = Food.ProductCode JOIN FoodType ON Food.FoodType = FoodType.ftID WHERE (Date BETWEEN ? AND ?) AND cinID = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setDate(1, dS);
            st.setDate(2, eS);
            st.setInt(3, cinID);
    ...
    public int getNumFoodTypeSellByDateATypeAC(Date dS, Date eS, String type, int cinID) {
        int a = 0, b = 0;
        try {
            String sql = "SELECT SUM(Quantity) AS T FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID JOIN Food ON OrderOnlineDetail.ProductCode = Food.ProductCode JOIN FoodType ON Food.FoodType = FoodType.ftID  JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND (PaymentDate BETWEEN ? AND ?) AND ftID = ? AND cinID = ?";

            String sql1 = "SELECT SUM(Quantity) AS T FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID JOIN Food ON OrderOfflineDetail.ProductCode = Food.ProductCode JOIN FoodType ON Food.FoodType = FoodType.ftID WHERE (Date BETWEEN ? AND ?) AND ftID = ? AND cinID = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setDate(1, dS);
            st.setDate(2, eS);
            st.setString(3, type);
    ...
    public int getNumFoodByOnlAndOffAC(Date dS, Date eS, String type, int cinID) {
        try {
            String sql = "";
            if (type.equals("ONL")) {
                sql = "SELECT SUM(Quantity) AS T FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND (PaymentDate BETWEEN ? AND ?) AND cinID = ?";
            } else if (type.equals("OFF")) {
                sql = "SELECT SUM(Quantity) AS T FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID WHERE (Date BETWEEN ? AND ?) AND cinID = ?";
            } else {
                return 0;
            }
            PreparedStatement st = connection.prepareStatement(sql);
    ...
    public int getNumFoodByDateEXACTAC(Date dS, Date eS, Date nS, int cinID) {
        int a = 0, b = 0;
        try {
            String sql = "SELECT SUM(Quantity) AS T FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID  JOIN TransactionCode ON TransactionCode.OrderID = OrderOnline.OrderID WHERE TransactionCode.Type = 1 AND PaymentDate = ? AND cinID = ?";
            String sql1 = "SELECT SUM(Quantity) AS T FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID WHERE Date = ? AND cinID = ?";
    ...
    public List<TIcketDate> getDateByNumTickDateAC(Date dS, Date eS, int cinID) {
        List<TIcketDate> list = new ArrayList<>();
        try {
            String sql = "SELECT DISTINCT PaymentDate FROM OrderOnlineDetail JOIN OrderOnline ON OrderOnlineDetail.OrderID = OrderOnline.OrderID JOIN Food ON OrderOnlineDetail.ProductCode = Food.ProductCode LEFT JOIN FoodType ON Food.FoodType = FoodType.ftID JOIN TransactionCode ON OrderOnline.OrderID = TransactionCode.OrderID JOIN Cinema ON TransactionCode.cinID = Cinema.cinID WHERE (PaymentDate  BETWEEN ? AND ?) AND TransactionCode.Type = 1 AND Cinema.cinID = ?\n"
                    + "UNION\n"
                    + "SELECT DISTINCT Date FROM OrderOfflineDetail JOIN OrderOffline ON OrderOfflineDetail.OrderID = OrderOffline.OrderID JOIN Food ON OrderOfflineDetail.ProductCode = Food.ProductCode LEFT JOIN FoodType ON Food.FoodType = FoodType.ftID JOIN Cinema ON OrderOffline.cinID = Cinema.cinID WHERE (Date BETWEEN ? AND ?) AND Cinema.cinID = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setDate(1, dS);
            st.setDate(2, eS);
            st.setInt(3, cinID);
