...
public class AddF extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddF</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddF at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (fdo.getFoodById(code) != null) {
            request.setAttribute("ms", "ID đã tồn tại");
            List<String> discontinued = new ArrayList<>();
            discontinued.add("Tiếp tục kinh doanh sản phẩm này");
            discontinued.add("Không kinh doanh sản phẩm này nữa");
            List<String> status = new ArrayList<>();
            ...
            status.add("HẾT HÀNG");
            request.setAttribute("status", status);
            request.setAttribute("type", fdo.getAllFoodType());
            request.setAttribute("discontinued", discontinued);
            request.getRequestDispatcher("addFood.jsp").forward(request, response);
        } else {
            int id = fdo.insertProductFood(price * 1000, dc);
            fdo.insertFood("FD" + id, fd, tl, stt, dis, img);
            Food f = fdo.getFoodById("FD" + id);
