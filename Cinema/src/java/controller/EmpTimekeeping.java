 ...
 * @author acer
 */
public class EmpTimekeeping extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EmpTimekeeping</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EmpTimekeeping at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        TimekeepingDAO tkdp = new TimekeepingDAO();
        ...
        if (request.getParameter("id") == null || request.getParameter("month") == null || request.getParameter("year") == null) {
            ...
            List<Timekeeping> list = tkdp.getTimekeepingByEmpAndYear(id, year, month);
            for (int j = 0; j < list.size(); j++) {
                String dateS = "", monthS = "", yearS = "";
                String t = list.get(j).getDate().toString();
                int cnt = 0;
                ...
                list.get(j).setDateS(dateS + "-" + monthS + "-" + yearS);
            }
            ...
            if (!list.isEmpty()) {
                ...
                request.setAttribute("ms", "Không có dữ liệu về chấm công của thời gian này");
            }
            request.getRequestDispatcher("empTpkd.jsp").forward(request, response);
        }
    }
